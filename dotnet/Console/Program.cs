...
class Program
{
    static int Main(string[] args)
    {
        ...
        Assembly calendar = Assembly.Load("Year2021");
        ...
        // for now.
        Type day = calendar.GetType($"Year2021.{dayArg}"/*, true*/) ??
            throw new Exception(
                $"Failed to get Year2021.{dayArg}. Not implemented?"
            );

        var input = System.IO.File.ReadLines($"Year2021/{dayArg}Input");
        MethodInfo inputParser = day.GetMethod("ParseInput") ??
            throw new Exception(
                $"Failed to get Year2021.{dayArg}.ParseInput. Not implemented?"
            );
        ...
        MethodInfo solver = day.GetMethod($"Solve{partArg}") ??
            throw new Exception(
                $"Failed to get Year2021.{dayArg}.{partArg}. Not implemented?"
            );
